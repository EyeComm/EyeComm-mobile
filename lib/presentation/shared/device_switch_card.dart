...
class DeviceSwitchCard extends StatelessWidget {
  ...
  final VoidCallback? onTap; // محتفظ به للـ backward compat لكن غير مستخدم
  final String stable;
  final int cd;
  ...
  const DeviceSwitchCard({
    ...
    this.statusText,
    this.onTap, // غير مستخدم - الإيماءات فقط
                ...
                Container(
                  ...
                  child: Center(
                    child: Image.asset(
                      ...
                      width: iconSize * 1.1,
                      height: iconSize * 1.1,
                      color: hasSwitch && !isDeviceOn ? kTextSub1 : (active ? activeColor : null),
                      colorBlendMode: BlendMode.srcIn,
                      errorBuilder: (_, __, ___) =>
                          Icon(Icons.device_unknown, color: activeColor, size: iconSize),
                    ),
                  ),
                ),
                SizedBox(height: vGap),
                Padding(
                  padding: EdgeInsets.symmetric(horizontal: box.maxWidth * 0.08),
                  child: Text(
                    label,
