  ...
  Widget build(BuildContext context) {
    ...
    return BaseGridPage(
      ...
      items: [
        ...
        {
          'eye': 'up',
          'text': ar ? 'عناية ونظافة' : 'Personal Care',
          'iconAsset': 'assets/icons/care.png',
          'color': Colors.teal,
          'is_nav': true,
          'eye_name': eyeName('up')
        },
        {
          'eye': 'down',
          'text': ar ? 'راحتي' : 'Comfort',
          'iconAsset': 'assets/icons/comfort.png',
          'color': Colors.purple,
          'is_nav': true,
      ...
      onAction: (eye, ctx) async {
        switch (eye) {
          case 'closed':
            VoiceService.speak(ar ? 'رجوع' : 'Going back');
            Navigator.pop(ctx);
            break;
          case 'left':
            await push(ctx, const FoodScreen());
            VoiceService.speak(ar ? 'طعام' : 'Food');
            break;
          case 'right':
            await push(ctx, const DrinksScreen());
            VoiceService.speak(ar ? 'مشروب' : 'Drink');
            break;
          case 'up':
            await push(ctx, const PersonalCareScreen());
            VoiceService.speak(ar ? 'عناية' : 'Care');
            break;
          case 'down':
            await push(ctx, const ComfortScreen());
            VoiceService.speak(ar ? 'راحة' : 'Comfort');
            break;
        }
      },
    );
