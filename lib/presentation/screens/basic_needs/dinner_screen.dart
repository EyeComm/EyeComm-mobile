      ...
      onAction: (eye, ctx) async {
        ...
        if (eye == 'left') {
          await push(ctx, SubItemsScreen(titleAr: 'بروتين خفيف', titleEn: 'Light Protein', options: [
            {'eye': 'left', 'text': ar ? 'علبة تونة' : 'Canned Tuna', 'iconAsset': 'assets/tuna.png', 'color': const Color(0xFF29B6F6), 'eye_name': eyeName('left')},
            {'eye': 'up', 'text': ar ? 'بيض مسلوق' : 'Boiled Eggs', 'iconAsset': 'assets/boiled_egg.png', 'color': const Color(0xFF0288D1), 'eye_name': eyeName('up')},
            {'eye': 'right', 'text': ar ? 'زبادي يوناني' : 'Greek Yogurt', 'iconAsset': 'assets/yogurt.png', 'color': const Color(0xFF039BE5), 'eye_name': eyeName('right')},
          ]));
        } else if (eye == 'up') {
          await push(ctx, SubItemsScreen(titleAr: 'شوربات دافئة', titleEn: 'Warm Soups', options: [
            {'eye': 'left', 'text': ar ? 'شوربة عدس' : 'Lentil Soup', 'iconAsset': 'assets/lentil.png', 'color': const Color(0xFFFFA726), 'eye_name': eyeName('left')},
            {'eye': 'up', 'text': ar ? 'شوربة خضار صافية' : 'Clear Veggie Soup', 'iconAsset': 'assets/veg_soup.png', 'color': const Color(0xFFF57C00), 'eye_name': eyeName('up')},
            {'eye': 'right', 'text': ar ? 'شوربة لسان عصفور' : 'Orzo Soup', 'iconAsset': 'assets/chicken_soup.png', 'color': const Color(0xFFFB8C00), 'eye_name': eyeName('right')},
          ]));
        } else if (eye == 'right') {
          await push(ctx, SubItemsScreen(titleAr: 'سلطات وعشاء خفيف', titleEn: 'Salads & Light Dinner', options: [
            {'eye': 'left', 'text': ar ? 'سلطة خضراء' : 'Green Salad', 'iconAsset': 'assets/green_salad.png', 'color': const Color(0xFF4CAF50), 'eye_name': eyeName('left')},
            {'eye': 'up', 'text': ar ? 'سلطة جبنة فيتا' : 'Feta Salad', 'iconAsset': 'assets/feta_salad.png', 'color': const Color(0xFF2E7D32), 'eye_name': eyeName('up')},
            {'eye': 'right', 'text': ar ? 'سلطة دجاج مشوي' : 'Grilled Chicken Salad', 'iconAsset': 'assets/chicken_salad.png', 'color': const Color(0xFF388E3C), 'eye_name': eyeName('right')},
          ]));
        } else if (eye == 'down') {
          await push(ctx, SubItemsScreen(titleAr: 'باقي الغداء', titleEn: 'Leftovers', options: [
            {'eye': 'left', 'text': ar ? 'توست خفيف' : 'Light Toast', 'iconAsset': 'assets/toast.png', 'color': const Color(0xFF5D4037), 'eye_name': eyeName('up')},
            {'eye': 'right', 'text': ar ? 'خبز' : 'Bread', 'iconAsset': 'assets/bread.png', 'color': const Color(0xFF795548), 'eye_name': eyeName('right')},
          ]));
        }
      },
    );
