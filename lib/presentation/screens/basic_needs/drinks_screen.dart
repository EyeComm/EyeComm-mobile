  ...
  Widget build(BuildContext context) {
    ...
    return BaseGridPage(
      ...
      onAction: (eye, ctx) async {
        ...
        if (eye == 'left') { VoiceService.speak(ar ? 'أريد أن أشرب ماء' : 'I want to drink water'); return; }

        if (eye == 'up') await push(ctx, _buildHotDrinks(ar));
        else if (eye == 'right') await push(ctx, _buildColdJuices(ar));
        else if (eye == 'down') await push(ctx, _buildDairyProtein(ar));
      },
    );
  }
